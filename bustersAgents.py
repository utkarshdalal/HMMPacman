...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDistributions in livingGhostPositionDistributions:
            positionProb = 0
            likelyPosition = None
            for p in ghostDistributions:
                if ghostDistributions[p] > positionProb:
                    positionProb = ghostDistributions[p]
                    likelyPosition = p
            dist = self.distancer.getDistance(pacmanPosition, likelyPosition)
            if dist < closestGhostDistance:
                closestGhostDistance = dist
                closestPosition = likelyPosition

        bestMove = None
        newDistance = closestGhostDistance
        for move in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, move)
            if self.distancer.getDistance(successorPosition, closestPosition) < newDistance:
                newDistance = self.distancer.getDistance(successorPosition, closestPosition)
                bestMove = move
        return bestMove
